    ...
    if (nomeCliente) {
      try {
        const response = await fetch(
          "http://localhost:3001/clienti/filtered?nomeContatto=" + nomeCliente,
          {
            method: "GET",
            headers: {
        ...
        if (!response.ok) {
          ...
          if (data.content && data.content.length > 0) {
            ...
            successNoty("Clienti caricati correttamente.");
          } else {
            setdataClienti([]);
            setStatoClienteRicerca(false);
            errorNoty("Nessun cliente trovato.");
          }
        } else {
          const error = await response.json();
          errorNoty(
            error.message ||
              `Errore durante il caricamento delle Province: ${response.status} ${response.statusText}`
          );
        }
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        const error = await response.json();
        notifyErrorAddFattura(
          error.message || "Errore durante l'aggiunta della fattura."
        );
      }
    } catch (error) {
    ...
    >
      <div className="container-fluid" style={{ marginBottom: "400px" }}>
        ...
        <form className="d-flex mt-4">
          ...
          <Button
            onClick={() => {
              fetchRicercaCliente(ricercaCliente);
            }}
            variant="contained"
          >
            Cerca Cliente
        ...
        {statoClienteRicerca && (
          <div className="d-flex mt-5">
            <Autocomplete
              ...
              renderInput={(params) => (
                <TextField {...params} label="Seleziona Cliente" />
              )}
            />
              ...
              <div className="mt-4 col-lg-4 col-md-6 col-sm-12">
                {isLoadingStatoFattura ? (
                  ...
                  <Autocomplete
                    ...
                    renderInput={(params) => (
                      <TextField {...params} label="Stato Fattura" />
                    )}
                  />
                )}
              </div>
        ...
        <div className="mt-5">
          <Button
            className="mt-4"
            onClick={() => {
              navigate("/aggiungistatofattura");
            }}
            variant="contained"
          >
            Aggiungi Stato fattura
          </Button>
        </div>
      </div>
    </motion.div>
